...
typedef struct struct_message
{
  ...
  double AngleErrorSup; // Diferença entre a aceleração em rad/s^2 medida e Angle
}struct_message;
...
void loop() 
{
  ...
  { // Move o robô para frente
    ...
    { // Move o robô para frente com PID
      if(abs(AngleError) > 0.20 && AngleError > 0)  // 0.20 é rads/s^2 (aceleração angular mínima escolhida para usar PID), AngleErro > 0 é condição com o eixo Z de cabeça para baixo
      {
        AngleErrorM1 = 0.00;
      ...
      else if(abs(AngleError) > 0.20 && AngleError < 0)  // 0.20 é rads/s^2 (aceleração angular mínima escolhida para usar PID), AngleErro < 0 é condição com o eixo Z de cabeça para baixo
      {
  ...
  myData.AngleErrorSup = AngleError;
